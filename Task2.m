...
for i = 1:length(names.one)
    ...
    [Time.two.(names.one(i)), Statevector.two.(names.one(i))] = ode45(@(t,statevector) QuadrotorEOM_Linearized(t,statevector,g,m,I,Body_motor(1),Body_motor(2:4)),tspan,Perturbation.one.(names.one(i)));
    ...
    PlotAircraftSim(Time.onetwo.(names.one(i)),Statevector.onetwo.(names.one(i)),Body_motor_time.onetwo.(names.one(i)),(1:6) +(i-1)*6,col,print_names.one(i),legend_names.one)
end
...
names.five = ["a" "b" "c" "d" "e" "f"];
print_names.five = ["2_5_a" "2_5_b" "2_5_c" "2_5_d" "2_5_e" "2_5_f"];
legend_names.five = ["Uncontrolled" "Controlled"];


for i = 1:length(names.five)
    % Calculating the response to the perturbations
    [Time.five.(names.five(i)), Statevector.five.(names.five(i))] = ode45(@(t,statevector) QuadrotorEOMwithRateFeedback(t,statevector,g,m,I,nu,mu),tspan,Perturbation.five.(names.one(i)));
    Statevector.five.(names.five(i)) = transpose(Statevector.five.(names.one(i)));
